...
import { Image, Eye, GripVertical, X, ChevronLeft, ChevronRight } from 'lucide-react'
      ...
      <div className="bg-green-50 border-b border-green-200 p-4 md:p-6 lg:p-8 relative">
        ...
        <div className="mb-4 pt-8 md:pt-10 lg:pt-12">
          <ListingMeta
            ...
            className="mb-2"
          />
          <h3 className="font-bold text-emerald-900 mb-2 text-lg md:text-xl lg:text-2xl listing-title">{title}</h3>
          <div 
            className="text-emerald-700 text-sm md:text-base lg:text-lg leading-relaxed listing-description"
            ...
            >
              ...
              {items[1]?.url ? (
                <img
                  src={items[1].url}
                  alt={items[1].name}
                  className="absolute left-0 top-0 z-10 w-full h-full object-cover select-none cursor-pointer"
                  style={{
              ...
              {items[0]?.url ? (
                <img
                  src={items[0].url}
                  alt={items[0].name}
                  className="absolute left-0 top-0 w-full h-full object-cover select-none cursor-pointer"
                  draggable={false}
            ...
            {items[lightbox.index] ? (
              <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-4 rounded-b-lg">
                <div className="text-white">
                  <h3 className="font-semibold text-lg">{items[lightbox.index]!.name}</h3>
                  {items[lightbox.index]?.price && (
                    <p className="text-emerald-400 font-bold text-xl">
                      R{items[lightbox.index]?.price?.toLocaleString('en-ZA', { minimumFractionDigits: 2 })}
                    </p>
                  )}
                  <p className="text-sm text-green-300 mt-1">
                    {lightbox.index + 1} of {items.length}
                  </p>
                </div>
              </div>
            ) : null}
