-- PayFast + EFT Payment System Setup (Fixed Version)
...
INSERT INTO subscription_pricing (tier, billing_cycle, amount_cents, discount_percentage) 
SELECT 'business', 'annual', 323892, 10
...
-- Create view for admin payment dashboard (replace if exists)
...
-- Add RLS policies for payment transactions (only if table exists)
DO $$
BEGIN
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'RLS policies setup completed with some warnings: %', SQLERRM;
END $$;
...
SELECT 'Tables: payment_transactions, eft_banking_details, subscription_pricing' as tables_created;
SELECT 'Functions: generate_payment_reference, activate_subscription' as functions_created;
SELECT 'View: admin_payment_overview' as views_created;
