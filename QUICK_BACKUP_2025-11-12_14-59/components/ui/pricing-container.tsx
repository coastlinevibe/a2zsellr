...
const Counter = ({ from, to }: { from: number; to: number }) => {
    ...
    React.useEffect(() => {
        ...
        const controls = animate(currentValue, to, {
            duration: 0.8,
            onUpdate(value) {
...
const PricingToggle = ({ isYearly, onToggle }: { isYearly: boolean; onToggle: () => void }) => (
    <div className="flex justify-center items-center gap-6 mb-8 relative z-10">
        <motion.button
            ...
            className={`px-6 py-3 font-black text-sm rounded-lg border-2 border-black transition-all ${
                !isYearly 
                    ? 'bg-blue-500 text-white shadow-[4px_4px_0px_0px_rgba(0,0,0,0.9)]' 
                    : 'bg-white text-black shadow-[2px_2px_0px_0px_rgba(0,0,0,0.9)] hover:shadow-[4px_4px_0px_0px_rgba(0,0,0,0.9)]'
            }`}
            whileHover={{ 
                scale: !isYearly ? 1 : 1.05,
        ...
        {isYearly && (
            <motion.span
                ...
                className="bg-yellow-400 text-black px-3 py-2 rounded-lg border-2 border-black font-black text-sm shadow-[3px_3px_0px_0px_rgba(0,0,0,0.9)]"
            >
                SAVE 20%!
            </motion.span>
        )}
...
const PricingCard = ({
    ...
    const previousPriceRef = useRef(plan.monthlyPrice);
    ...
    // Update previous price ref after render
    React.useEffect(() => {
...
// Export the PricingPlan interface for use in other files
